...
export class Logs {
	...
	static tryCatch(func: any, options: object): boolean {
		try {
			...
			if (process.env.SF_ENV_LOGS === "2" || process.env.SF_ENV_LOGS === "4") console.log(Logs.logLevel(options));
			else console.log(Logs.logLevel(err));
	...
	static receiveError(error: any) {
		...
		else console.log(this.logLevel(error));
	}

	static receiveSuccess(success: any) {
		if (process.env.SF_ENV_LOGS === "3" || process.env.SF_ENV_LOGS === "5") console.log(this.logLevel(success));
	}
	...
	private static logLevel(log: any): string {
		switch (parseInt(process.env.SF_ENV_LOGS)) {
			...
			case 4:
				new Logs().logExist(log.error);
				return log.error;

			case 5:
				new Logs().logExist(log);
				return log;
