...
export class MainModel {
	...
	protected addMainModel(options: IOptions, paramsArray: any): Promise<any> {
		return new Promise((resolve, reject) => {
			DB.simpleQuery(options, this.prepareDataforInsert(options, paramsArray))
	...
	protected findAllMainModel(options: IOptions): Promise<any> {
		return new Promise((resolve, reject) => {
			DB.simpleQuery(`SELECT ${options.field} FROM ${options.table}`, options)
				.then((res: Response) => resolve(res))
				.catch((err: Response) => reject(err));
		});
	}
	...
	protected findMainModelWithMultipleCondition(options: IOptions, paramsArray: any): Promise<any> {
		return new Promise((resolve, reject) => {
			DB.simpleQuery(this.prepareDataCondition(options, paramsArray), options)
				.then((res: Response) => resolve(res))
				.catch((err: Response) => reject(err));
		});
	}
	...
	private prepareDataCondition(options: IOptions, paramsArray: any): any {
		...
		for (let key in paramsArray) {
			if (i < Object.keys(paramsArray).length) data = data + key + "=" + '"' + paramsArray[key] + '"' + " && ";
			else {
    ...
    */
		...
		return new Promise((resolve, reject) => {
			return new Promise((resolve, reject) => {
				this.findMainModel(options)
					.then(() => {
						DB.simpleQuery(this.prepareDataUpdate(options, paramsArray), options)
							//res.affectedRows >= 1
							.then((res: Response) => resolve(res))
							.catch((err: Response) => reject(err));
					})
	...
	protected verifBcrypt(params: any, hash: any): boolean {
		if (bcrypt.compareSync(options.sf_salt + params, hash)) return true;
		else return false;
	}
	...
	protected executePersonnalRequest(options: IOptions): Promise<any> {
		return new Promise((resolve, reject) => {
			DB.simpleQuery(options.requestSQL, options)
				.then((res: Response) => resolve(res))
				.catch((err: Response) => reject(err));
		});
	}
